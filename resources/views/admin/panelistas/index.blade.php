    ...
    @if(Auth::user()->hasRole('admin'))
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="w-full block">
                <a class="verde-lima-bg text-white font-semibold py-2 px-4 border border-gray-400 rounded shadow" href="{{ route('panelistas.create') }}"> Insertar nuevo</a>
            </div>
            ...
            <div class="bg-white shadow-md rounded my-6">
                <table class="w-full my-5 border-solid border-gray-700">
                    ...
                    <tr class="text-center">
                        ...
                        <td class="px-4 py-3"><strong class="verde-lima-color">{{ $value->nombre }}</strong></td>
                        <td class="px-4 py-3">{{ \Str::limit($value->hv, 25) }}</td>
                        <td class="px-4 py-3">{{ $value->telefono }}</td>                    
                        
                        <td class="py-3">
                            <form action="{{ route('panelistas.destroy',$value->id) }}" method="POST">
                                
                                <a class="azul-andes-bg  text-white  py-2 px-4 border border-gray-400 rounded shadow" href="{{ route('panelistas.show',$value->id) }}">Mostrar</a>    
                                <a class="verde-lima-bg text-white  py-2 px-4 border border-gray-400 rounded shadow" href="{{ route('panelistas.edit',$value->id) }}">Editar</a>   
                                @csrf
                                @method('DELETE')      
